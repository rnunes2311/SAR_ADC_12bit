...
if length(cppsim_home) == 0
   disp('Error running ngsim Matlab script:  environment variable');
...
run_program = sprintf('%s/NGspice/bin/ngspice -b -r simrun.raw -o simrun.log simrun.sp',cppsimshared_home);
[status] = system(run_program);
